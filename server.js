      ...
      docs.forEach((doc) => {
        ...
        doc.fixedDipositEntery.forEach((fixded) => {
          if (fixded.maturitiyDate === getOnlyDate()) {
            ...
            db.updateCurentTotalAmount(accountNo, currentTotalAmount)
              .then((customers) => {
                let day = getOnlyDate();
                let dayNumberk = dayNumber(day);
                db.TransactionDeatailsSave(accountNo, "Unlock The Fixed Amount", parseInt(fixded.finalAmount), currentTotalAmount, getDate(), "auto", "Deposit", dayNumberk, fixded.BankerId)
                  .then((foundCustomer) => {
        ...
        if (loanEMI.dateIntarestCalculate === getOnlyDate()) {
          ...
          db.updateCurentTotalAmount(accountNo, currentTotalAmount)
            .then((posts) => {
              let day = getOnlyDate();
              let dayNumberk = dayNumber(day);
              db.TransactionDeatailsSave(accountNo, "EMI", parseInt(loanEMI.loanEmiAmount), currentTotalAmount, getDate(), loanEMI.branchName, "Withdrawal", dayNumberk, loanEMI.BankerId)
                .then((foundCustomer) => {
                  ...
                  db.updateRunningEMI(accountNo, emiId, totalPayable, dateIntarestCalculate, noInstallments)
                    .then((posts) => {
                      if (noInstallments === 0) {
...
app.post("/checkingOTP", (req, res) => {
  if (req.body.userOtp == req.body.serverOTP) {
    ...
    userRegister(accountNo, passw, "customer")
      .then((posts) => {
        console.log("sucess");
...
app.post("/memberRegister/:urlUser", uploadMultiple, function (req, res, next) {
  ...
  db.newCustomerEntry( req.body.fName, req.body.mName, req.body.lName, req.body.ffName, req.body.fmName, req.body.flName, req.body.gender, req.body.marriageStatus, req.body.address, req.body.phone, req.body.pin, req.body.panNo, req.body.voter, req.body.adhar, req.body.email, req.body.dob, req.body.occupation, req.body.nominiName, req.body.nominiRelation, decriptedBankerId, req.files.passportSizePhoto[0].buffer, req.files.Signature[0].buffer )
  .then((item)=>{
...
app.post("/deposit/:bankerId/:accountno/save", (req, res) => {
  ...
  let day = getOnlyDate();
  let dayNumberk = dayNumber(day);
   db.updateCurentTotalAmount( deRequestedAccountNo , urrentTotalAmount )
   .then((posts)=>{
     ...
     db.updateCurentTotalAmount( deRequestedAccountNo , currentTotalAmount )
        .then((posts)=>{      
          ...
          db.TransactionDeatailsSave( deRequestedAccountNo , "Cut to Clear The Full Loan Amount" , parseInt(req.body.totalAmountwithIntarest) , currentTotalAmount , getDate() , req.body.branchName , "withdrawal" , dayNumberk , deBankerId )
            .then(( foundCustomer )=>{
              db.deleteLoanSubDoc( req.body.AccountNo, req.body.loanId )
              .then((posts)=>{
...
app.post("/forgetpassword/checkingOTP", (req, res) => {
  if (req.body.userOtp == req.body.serverOTP) {
    ...
    let passw = req.body.passWord;
          ...
          .then((posts)=>{
            db.userRegister( accountNo, passw, "customer" )
            .then((posts)=>{
              res.render("sucessRegisted", {
                passwordUpdate: "yes",
                signUp: " "
